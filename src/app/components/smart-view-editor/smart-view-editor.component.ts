...
export class SmartViewEditorComponent implements OnInit, OnDestroy {
  @ViewChild('gridFields', { static: false, read: GridComponent })
  gridFields: GridComponent;
  @ViewChild('gridColumns', { static: false, read: GridComponent })
  gridColumns: GridComponent;
  @ViewChild('columnEdit', {
    static: false,
    read: SmartViewEditorColumnComponent,
  })
  columnEdit: SmartViewEditorColumnComponent;
  ...
  public openModal(
    ...
    action: 'newview' | 'editview' | 'dupeview',
    newTarget: string = ''
  ): void {
    if (newTarget != '') {
      this.propertyList = [];
  ...
  public gridFieldsDoubleClick(event: any) {
    ...
    if (index >= 0) {
      ...
      let column: IColumn = {
        id: `${this.propertyList[this.selectedTableIndex].table}-${
          rowData.fieldNumber
        }`,
  ...
  public gridColumnsDoubleClick(event: any) {
    ...
    let index = this.viewData.detail.editColumns.findIndex((e) => {
  ...
  private populateFields(): void {
    ...
    this.propertyList[this.selectedTableIndex].fields.forEach((field) => {
      let idx = this.viewData.detail.editColumns.findIndex((column) => {
        return (
          field.fieldNumber === column.fieldNumber &&
          this.propertyList[this.selectedTableIndex].table === column.table
        );
      });
